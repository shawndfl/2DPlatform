...
export class TileFactory extends Component {
    ...
    static parseTile(tile: string): ITileTypeData {
        ...
        const TileData: ITileTypeData = { tile, tileClass: '', spriteId: '', options: null, typeIndex: -1 }
     ...
     * @returns
     */
    create(tile: string, i: number, j: number, k: number): TileComponent {

        if (!tile || tile == '---' || tile == 'empty') {
            return new EmptyTile(this.gm, i, j, k);
        }
        ...
        const args: ITileCreationArgs = {
            i, j, k, tileClass: data.tileClass, spriteName: data.spriteId, options: data.options
        }
