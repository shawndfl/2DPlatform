...
export abstract class TileComponent extends GameComponent {
  ...
  public get screenBounds(): Readonly<rect> {
    ...
    this._screenBounds.top =
      this._screenPosition.y + this.spriteController.spriteHeight();
  ...
  constructor(
    ...
    protected _tileData: ITileCreationArgs
  ) {
    super(_groundManager.eng);
    this._id = this.createTileId('tile', this._tileData?.i, this._tileData?.j);
   ...
   * @returns
   */
  createTileId(prefix: string, i: number, j: number): string {
    if (!prefix) {
      prefix = 'tile';
    }
    return prefix + '.' + (i ?? '_') + '.' + (j ?? '_');
  }
   ...
   * @returns
   */
  TileToScreen(i: number, j: number, screen?: vec3): vec3 {
    if (!screen) {
      screen = new vec3();
    }
   ...
   * @param k
   */
  setTilePosition(i: number, j: number) {
    this.TileToScreen(i, j, this.screenPosition);
